...
#define DECLARE_RET \
  int retExclusiveAaAa = 0

#define LOCK()                                                  \
  do {                                                          \
    if ((retExclusiveAaAa = pthread_mutex_lock(&mutex)) != 0) { \
      errExitEN(retExclusiveAaAa, "pthread_mutex_lock");        \
    }                                                           \
  } while (0)
#define UNLOCK()                                                  \
  do {                                                            \
    if ((retExclusiveAaAa = pthread_mutex_unlock(&mutex)) != 0) { \
      errExitEN(retExclusiveAaAa, "pthread_mutex_unlock");        \
    }                                                             \
  } while (0)
