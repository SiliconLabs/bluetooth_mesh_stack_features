...
/***************************************************************************//**
* Handle --exclude functionality
*
*******************************************************************************/
static void handle_exclude(void);
...
/// Flag stating that the btmesh_prov_initialized event has arrived
...
sl_status_t btmesh_app_prov_init(int opt, char *optarg)
{
  ...
  switch (opt) {
    ...
    case 'v': {
      // Exclude node from network
      app_log_filter_threshold_set(APP_LOG_LEVEL_INFO);
      app_log("Exclude: %s" APP_LOG_NEW_LINE, optarg);
      char *pos = optarg;
      size_t len = strlen(optarg);
      // Parse input for UUID
      app_parse_uuid(pos, len, &command_uuid);
      command = EXCLUDE_NODE;
      command_state = START;
      break;
    }
    case 'k': {
...
void handle_provision(void)
{
  ...
  switch (command_state) {
    ...
    case IN_PROGRESS:
      ...
      if (btmesh_app_prov_get_cbp_status() == true) {
    ...
    case START: {
      ...
      if ((true == param_ready) && (true == db_ready)) {
        ...
        if (SL_STATUS_OK != sc) {
          ...
          app_log_info("Node exclusion succeeded" APP_LOG_NEW_LINE);
        }
        command_state = FINISHED;
      }
      break;
...
SL_WEAK bool btmesh_app_prov_get_cbp_status(void)
{
  return false;
}
