...
#define OPTIONS                                                                 \
  ...
  "    -x --exclude     Exclude the given node from the Mesh network\n"         \
...
void app_init(int argc, char *argv[])
{
  ...
  int option_index = 0;
  while ((opt = getopt_long(argc, argv, "ehi:lp:r:x:s" OPTSTRING, long_options, &option_index)) != -1) {
    switch (opt) {
      case 'p':
...
void handle_provision(void)
{
  switch (command_state) {
    case START:
      ...
      sc = btmesh_prov_provision_adv_device(netkey_index, command_uuid, mac, 0, 0);
