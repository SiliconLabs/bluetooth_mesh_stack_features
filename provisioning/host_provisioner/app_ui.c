...
void handle_ui(void)
{
  switch (command_state) {
    case UI_START: {
      ...
      app_log("6. Exclude node from the network" APP_LOG_NEW_LINE);
      app_log_nl();
      ...
      if (true == input_ready) {
        ...
        switch (c) {
          ...
          case '5':
            ...
            break;
          case '6':
            set_command_and_state_ui(EXCLUDE_NODE, UI_START);
            break;
          case '9':
...
void handle_ui_exclude(void)
{
  switch (command_state) {
    case UI_START:
      if (WAITING_FOR_INPUT != ui_state) {
        app_log("Select a node to exclude" APP_LOG_NEW_LINE);
        app_log("Type either the ID, UUID or address from the list below" APP_LOG_NEW_LINE);
        app_log_nl();
        sl_status_t sc = btmesh_prov_list_provisioned_nodes();
        if (SL_STATUS_EMPTY == sc) {
        ...
        if (true == input_ready) {
          ...
          if (ADDRESS_LEN_WITHOUT_PREFIX > len) {
            ...
            sc = sl_btmesh_prov_set_key_refresh_exclusion(netkey_index,
                                                          1,
                                                          prov->uuid);
            app_assert_status_f(sc, "Exclusion failed" APP_LOG_NEW_LINE);
            app_log("Exclusion succeeded %d" APP_LOG_NEW_LINE, sc);
            ...
            uint16_t netkey_index = APP_NETKEY_IDX;
            btmesh_db_node_t *node = btmesh_db_node_get_by_addr(addr);
            btmesh_db_node_prov_t *prov = &node->prov;
            sc = sl_btmesh_prov_set_key_refresh_exclusion(netkey_index,
                                                          1,
                                                          prov->uuid);
            app_assert_status_f(sc, "Exclusion failed" APP_LOG_NEW_LINE);
