...
uint8_t netkey_id = KEY_INDEX_INVALID;//0xff;
uint8_t appkey_id = KEY_INDEX_INVALID;//0xff;
uint8_t ask_user_input = false;
uint16 provisionee_address = ADDRESS_INVALID;//0xFFFF;
...
static void  button_poll()
{
  ...
  if (GPIO_PinInGet(BSP_BUTTON1_PORT, BSP_BUTTON1_PIN) == 0 || key == 'y') {
    ...
    struct gecko_msg_mesh_prov_provision_device_with_address_rsp_t *prov_resp_adv;
    uint16 address_to_provision = ADDRESS_ASSIGN_AUTO;
    uint8 attention_time_seconds = PROVISION_ATTN_TIME_DISABLED;
    uint8 elements = 3; //not really sure that this works

    //prov_resp_adv = gecko_cmd_mesh_prov_provision_device(netkey_id, 16, _uuid_copy);
    prov_resp_adv = gecko_cmd_mesh_prov_provision_device_with_address(netkey_id,
                                                                  address_to_provision,
                                      elements,
                                      attention_time_seconds,
                                                                  sizeof(_uuid_copy), _uuid_copy);

    if (prov_resp_adv->result == STATUS_OK) {
      state = provisioning;
    } else {
...
static void config_retry(uint8 timer_handle){
  ...
  gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(500), timer_handle, SOFT_TIMER_REPEATING);
  ...
  switch (evt_id) {
    case gecko_evt_system_boot_id:
      ...
      if ((GPIO_PinInGet(BSP_BUTTON0_PORT, BSP_BUTTON0_PIN) == 0) || (GPIO_PinInGet(BSP_BUTTON1_PORT, BSP_BUTTON1_PIN) == 0)) {
        ...
        if (prov_init_rsp->result == STATUS_OK) {
    ...
    case gecko_evt_hardware_soft_timer_id:
      ...
      case TIMER_ID_GET_DCD:
      {
        ...
        struct gecko_msg_mesh_config_client_get_dcd_rsp_t* get_dcd_result = gecko_cmd_mesh_config_client_get_dcd(netkey_id, provisionee_address, PAGE0);
        if (get_dcd_result->result == STATUS_BUSY) {
          printf("."); fflush(stdout);
          gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(1000), TIMER_ID_GET_DCD, SOFT_TIMER_REPEATING);
      ...
      case TIMER_ID_APPKEY_BIND:
      {
        ...
        if(model_app_bind_result->result  == STATUS_OK)
        {
          ...
          printf("."); fflush(stdout);
          gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(500), TIMER_ID_APPKEY_BIND, SOFT_TIMER_REPEATING);
      ...
      case TIMER_ID_PUB_SET:
      {
        ...
        if (model_pub_set_result->result == STATUS_OK)
        {
          ...
          printf("."); fflush(stdout);
          gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(500), TIMER_ID_PUB_SET, SOFT_TIMER_REPEATING);
      ...
      {
        uint16 vendor_id = VENDOR_ID_INVALID;//0xFFFF;
        uint16 model_id;
        uint16 sub_address;
    ...
    case gecko_evt_le_connection_opened_id:
      ...
      if(state == connecting)
      {
        uint8 attention_time_seconds = PROVISION_ATTN_TIME_DISABLED;
        uint16 address_to_provision = ADDRESS_ASSIGN_AUTO;
        uint8 elements = 2;  //magic number 2
        ...
        struct gecko_msg_mesh_prov_provision_gatt_device_with_address_rsp_t *prov_resp_adv;
        //16?
        //prov_resp_adv = gecko_cmd_mesh_prov_provision_gatt_device(netkey_id, conn_handle, 16, _uuid_copy);
        prov_resp_adv = gecko_cmd_mesh_prov_provision_gatt_device_with_address(netkey_id,
                                                                           conn_handle,
                                         address_to_provision,
                                         elements, //not sure yet how to set this. How does the provisioner know how many elements the device has?
                                         attention_time_seconds,
                                         sizeof(_uuid_copy), _uuid_copy);
...
#define bearer_type 0
    ...
    case gecko_evt_mesh_prov_provisioning_failed_id:
    {
      ...
      printf("Provisioning failed. Reason: %2x\r\n", fail_evt->reason);
      state = scanning;
